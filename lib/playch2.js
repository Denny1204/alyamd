...
class SaveTube {
  ...
  async download(url) {
    ...
    if (!cdn.status) return { status: false }
...
async function playCh2(hydro, text, reply) {
  try {
    ...
    const data = await ytdl.download(video.url)
    if (!data.status) return reply("❌ Gagal mengambil audio.")

    const audioResponse = await axios.get(data.dl, {
    ...
    await new Promise((resolve, reject) => {
      exec(
        `ffmpeg -y -i "${tempInput}" -c:a libopus -b:a 128k -vbr on -compression_level 10 "${tempOutput}"`,
    ...
    await reply(`✅ Lagu *${data.title}* berhasil dikirim ke channel 🎶`)
